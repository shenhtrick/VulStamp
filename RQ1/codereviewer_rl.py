...
use_cuda = True
model_name = "t5"
pretrainedmodel_path = "../../models/codereviewer"
...
for epoch in range(num_epochs):
    ...
    for step, inputs in enumerate(batch_progress_bar):
        ...
        with torch.no_grad():
            ...
            # rewards = (sampled_actions == labels).float() * 2 - 1
        ...
        # log_probs = torch.log(probs[train_indices].gather(1, sampled_actions.unsqueeze(1))).squeeze()
    ...
    valid_results.to_csv('result/softprompt_rl(0.01_0.7)_difweight_(slicode_exp_imp_scop)/validation_results_{}.csv'.format(epoch), index=False)
